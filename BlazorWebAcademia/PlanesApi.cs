...
namespace BlazorWebAcademia_PlanesApi
{
    public class PlanesApi
    {
        ...
        static PlanesApi()
        {
            _planes.BaseAddress = new Uri("https://localhost:7111/");
        ...
        public static async Task<Plan> GetAsync(int id)
        {
            ...
            {
                plan = await response.Content.ReadFromJsonAsync<Plan>();
            }
            return plan;
        }
        ...
        public static async Task<IEnumerable<Plan>> GetAllAsync()
        {
            ...
            HttpResponseMessage response = await _planes.GetAsync("planesInstancia");
            if (response.IsSuccessStatusCode)
            {
                planes = await response.Content.ReadFromJsonAsync<IEnumerable<Plan>>();
            }
            return planes;
        }
